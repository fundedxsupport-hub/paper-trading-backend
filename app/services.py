...
from app.utils import clean_dict, day_bounds_utc, new_uuid, now_utc
...
def place_trade(payload: PlaceTradeRequest) -> dict[str, Any]:
    entry_price = get_market_price(payload.symbol, payload.market_price)
    timestamp = now_utc()
    trade = {
        "trade_id": new_uuid(),
...
def close_trade(payload: CloseTradeRequest) -> dict[str, Any]:
    ...
    wallets.update_one(
        {"client_id": payload.client_id},
        {
            "$inc": {"virtual_capital": final_pl, "realized_pl": final_pl},
            "$set": {"updated_at": timestamp},
        },
    )
